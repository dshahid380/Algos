        	...
        	if(j!=m) cout<<-1<<endl;
        		...
        		rep(i,0,sz(v)){
        			...
        			if(i==sz(v)-1){
        				...
        				sort(a.begin()+v[i].ss+1, a.begin()+v[i+1].ss);
        				tm1 += x * ((v[i+1].ss-v[i].ss-1)/k);
        				if((v[i+1].ss-v[i].ss-1)%k!=0){
        					if(max(a[v[i].ss],a[v[i+1].ss])>a[v[i+1].ss-1]){
        						tm1 += ((v[i+1].ss-v[i].ss-1)%k)*y;
        						tm2 += (v[i+1].ss-v[i].ss-1)*y;
        					}else f = 1;
        				}else{
        					if(max(a[v[i].ss],a[v[i+1].ss])>a[v[i+1].ss-1]){
        						tm2 += (v[i+1].ss-v[i].ss-1)*y;
